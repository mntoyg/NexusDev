...
**Good first contributions:** the seeded tasks in [`context/TODO.md`](context/TODO.md) are real and unclaimed — TASK-001 (a `STATE.json` schema and validator) and TASK-003 (`CONTRIBUTING.md`) are both `ready`. Beyond those: add a router backend, write ADRs, or port `ai-router.sh` to PowerShell for Windows contributors.
